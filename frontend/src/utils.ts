...
export async function searchWikimedia(make: string, model: string, year: string | null): Promise<WikiResult> {
  ...
  try {
    ...
    clearTimeout(id);
    if (!res.ok) return { success: false, message: `HTTP ${res.status}`, total: 0, images: [] };
    const text = await res.text();
    let data: any;
    try { data = JSON.parse(text); } catch {
      return { success: false, message: `Respuesta inválida`, total: 0, images: [] };
    }
    if (data?.error) return { success: false, message: `API: ${data.error.info ?? ''}`, total: 0, images: [] };
    const pages: any[] = data?.query?.pages;
    if (!pages || pages.length === 0) return { success: false, message: 'Sin resultados', total: 0, images: [] };
    const images: WikiImage[] = pages.map((p: any) => {
      const info = p?.imageinfo?.[0] ?? {};
      const meta = info.extmetadata ?? {};
      const author = meta.Artist?.value?.replace(/<[^>]+>/g, '').trim() || 'Autor desconocido';
      const license = meta.LicenseShortName?.value || 'CC BY-SA';
      return {
        id: p.pageid, title: p.title.replace('File:', ''),
        url: info.thumburl ?? info.url, width: info.thumbwidth ?? info.width ?? 0,
        height: info.thumbheight ?? info.height ?? 0, author, license,
        pageUrl: `https://commons.wikimedia.org/wiki/${p.title}`,
        attribution: `${p.title.replace('File:', '')} | ${author} | ${license}`,
      };
    });
    return { success: true, total: images.length, images };
  } catch (e: any) {
    clearTimeout(id);
    return { success: false, message: e?.name === 'AbortError' ? 'Tiempo agotado' : (e?.message ?? 'Error de red'), total: 0, images: [] };
  }
}

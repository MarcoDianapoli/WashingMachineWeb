...
export default function AdminWashers() {
  const { washers, appointments, addWasher, editWasher } = useApp();
  ...
  const getStats = (washerId: string) => {
    const washerAppointments = appointments.filter(a => a.washerId === washerId);
    const completed = washerAppointments.filter(a => a.status === 'completed').length;
    const pending = washerAppointments.filter(a => ['pending', 'confirmed', 'in_progress'].includes(a.status)).length;
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    editWasher(selectedWasher.id, updates);
    // Update local state so the modal reflects changes immediately
    setSelectedWasher({ ...selectedWasher, ...updates });
    setIsEditing(false);
  };
  ...
  const startEditing = () => {
    if (!selectedWasher) return;
    setFormName(selectedWasher.name);
    setFormPhone(selectedWasher.phone || '');
    setFormSpecialty(selectedWasher.specialty || '');
    setFormNotes(selectedWasher.notes || '');
    setIsEditing(true);
    setShowPerformance(false);
  };
  ...
  return (
    <div className="page" style={{ maxWidth: '1200px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
        {washers.map(washer => {
          const stats = getStats(washer.id);
          const isBusy = stats.pending > 0;
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
                ...
                <div>
                  <h3 style={{ fontSize: '1.2rem', margin: '0 0 4px 0', color: 'var(--black)' }}>{washer.name}</h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', borderTop: '1px solid var(--light-gray)', paddingTop: '16px' }}>
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--black)' }}>{stats.completed + Math.floor(Math.random() * 3)}</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: 'var(--gray)', fontSize: '0.85rem', marginBottom: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                    <Clock size={14} /> Pendientes
                  </div>
                  <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--black)' }}>{stats.pending}</div>
                </div>
              </div>
            </div>
          );
      ...
      {/* Washer details / edit modal */}
      {selectedWasher && (
        <div className="modal-overlay" onClick={() => setSelectedWasher(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
            <div className="modal-header" style={{ marginBottom: '20px' }}>
              <h2 className="modal-title">
                {isEditing ? 'Editar Lavador' : (showPerformance ? 'Resumen de Rendimiento' : 'Perfil del Lavador')}
              </h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                {!isEditing && !showPerformance && (
                  <button
                ...
                <button className="modal-close" onClick={() => setSelectedWasher(null)}>
                  <X size={24} />
                </button>
              </div>
            </div>
            ...
            {isEditing ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', paddingBottom: '20px', borderBottom: '1px solid var(--light-gray)' }}>
                  ...
                  <div>
                    <h2 style={{ margin: 0, fontSize: '1.5rem', color: 'var(--black)' }}>{selectedWasher.name}</h2>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--gray)', marginTop: '8px' }}>
                      <Phone size={16} /> {selectedWasher.phone || 'Sin número registrado'}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  <div className="card" style={{ padding: '20px', backgroundColor: 'var(--light-gray)' }}>
                    ...
                    <p style={{ margin: 0, color: 'var(--black)' }}>{selectedWasher.specialty || 'Lavado General'}</p>
                  </div>
                  <div className="card" style={{ padding: '20px', backgroundColor: 'var(--light-gray)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--gray)', marginBottom: '8px', fontWeight: 600 }}>
                      <Info size={18} /> Observaciones
                    </div>
                    <p style={{ margin: 0, color: 'var(--black)' }}>{selectedWasher.notes || 'Sin observaciones.'}</p>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <p style={{ color: 'var(--gray)', marginBottom: '10px' }}>
                  Estadísticas de rendimiento para <strong>{selectedWasher.name}</strong> durante este mes.
                </p>
